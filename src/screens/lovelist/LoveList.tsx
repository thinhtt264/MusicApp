...
import { useTrackFromPlayList } from 'src/common/firebase';
...
const LoveListScreen = () => {
  ...
  useTrackFromPlayList(playlistId, ({ data, totalItem }) => {
    setListTrack(data);
      ...
      <View style={styles.body}>
        ...
        <AnimatedList
          ...
          renderItem={({ item }: any) => (
            <TrackCard
              item={item}
              onOpenModal={(selectItem: any) => onOpenModal(selectItem)}
            />
          )}
...
const ItemSeparator = () => <Spacer size={scale(12)} />;
